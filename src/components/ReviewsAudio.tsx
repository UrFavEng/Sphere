...
import { ReviewAudio } from "@/app/store/types";
...
interface ReviewsAudioProps {
  setReviewsAudio: (vla: boolean) => void;
  idAudio: string;
  reviews: ReviewAudio[];
}
const ReviewsAudio = ({
  ...
  const onSubmit: SubmitHandler<data> = (data) => {
    if (data.comment.length !== 0) {
      const body = {
        ...
        rating,
        reviewed: `${idAudio}${userData?.documentId}`,
        audioo: idAudio,
