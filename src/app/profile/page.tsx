...
const Profile = () => {
  ...
  const { data, isLoading: loadinGetMe } = useGetMeQuery();
  const [showMore, setShowMore] = useState<boolean>(false);
  const [editPropfile, setEditPropfile] = useState<boolean>();
  const { data: videos, isLoading: loadingVideos } = useGetMeVideosQuery();
  const { data: audios, isLoading: loadingAudios } = useGetMeAudiosQuery();
  ...
  return (
    <div className=" pb-4">
      {isLoading ? (
        ...
        <>
          {isError ? (
            <p className=" text-[24px] font-bold text-center mt-12 text-red-600 ">
              ...
              <div className="content-start px-4 xl:px-0 container mx-auto  py-6 grid gap-4 md:gap-0  md:grid-cols-3 xl:grid-cols-4 ">
                ...
                <div className="w-full col-span-2 justify-self-center md:block hidden ">
                  ...
                  <section className="section">
                    {loadinGetMe || loadingAudios || loadingVideos ? (
                      <>
                        {" "}
                        <p className="text-center mt-4">
                          {" "}
                        ...
                        {section === "Audios" && (
                          <>
                            ...
                            {audios?.audioos?.map(
                              (aud) =>
                                !aud.publishedAt && (
                                  <div key={aud.documentId} className="w-full">
                                    <Audio audio={aud} />
                                  </div>
                                )
                            )}
